	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if storageAccountId, err := commonids.ParseStorageAccountID(model.TargetResourceId); err == nil {
				targetResourceId := fmt.Sprintf("%s/blobService/default", storageAccountId.ID())
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model := resp.Model; model != nil {
				...
				if props.SecretStore != nil {
					state.SecretStore = flattenSecretStore(pointer.From(props.SecretStore))
				}
	...
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if d.HasChange("secret_store") {
				secretStore := expandSecretStore(state.SecretStore)
				linkerProps.SecretStore = (*links.SecretStore)(secretStore)
			}
