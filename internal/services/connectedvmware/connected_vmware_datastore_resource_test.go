...
func (r ConnectedVmwareDatastoreResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	id, err := datastores.ParseDataStoreID(state.ID)
	if err != nil {
		return nil, err
	}

	resp, err := client.ConnectedVmware.DataStoresClient.Get(ctx, *id)
	if err != nil {
...
func (r ConnectedVmwareDatastoreResource) basic(data acceptance.TestData) string {
...
func (r ConnectedVmwareDatastoreResource) template(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
