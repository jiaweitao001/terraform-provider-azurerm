...
func (r ConnectedVmwareVcenterResource) basic(data acceptance.TestData, credential, privateKey, publicKey string) string {
...
resource "azurerm_connected_vmware_vcenter" "test" {
  ...
  extended_location {
	name = azurerm_extended_custom_location.test.id
...
func (r ConnectedVmwareVcenterResource) template(data acceptance.TestData, credential, publicKey, privateKey string) string {
...
resource "azurerm_extended_custom_location" "test" {
  name                = "acctestcustomlocation%[1]d"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_extension_ids = [
    azurerm_arc_kubernetes_cluster_extension.test.id
  ]
  namespace        = "namespace%[1]d"
...
func (r ConnectedVmwareVcenterResource) generateKey() (string, string, error) {
...
func (r ConnectedVmwareVcenterResource) getCredentials(t *testing.T) (credential, privateKey, publicKey string) {
...
func (r ConnectedVmwareVcenterResource) provisionTemplate(data acceptance.TestData, credential string, privateKey string) string {
	return fmt.Sprintf(`
connection {
 type     = "ssh"
