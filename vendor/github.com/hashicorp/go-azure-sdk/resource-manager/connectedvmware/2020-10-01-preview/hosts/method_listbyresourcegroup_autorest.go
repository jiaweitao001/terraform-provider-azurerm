...
func (c HostsClient) ListByResourceGroup(ctx context.Context, id commonids.ResourceGroupId) (resp ListByResourceGroupOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "hosts.HostsClient", "ListByResourceGroup", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "hosts.HostsClient", "ListByResourceGroup", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByResourceGroup(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "hosts.HostsClient", "ListByResourceGroup", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c HostsClient) preparerForListByResourceGroup(ctx context.Context, id commonids.ResourceGroupId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/providers/Microsoft.ConnectedVMwarevSphere/hosts", id.ID())),
...
func (c HostsClient) preparerForListByResourceGroupWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c HostsClient) responderForListByResourceGroup(resp *http.Response) (result ListByResourceGroupOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByResourceGroupOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "hosts.HostsClient", "ListByResourceGroup", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByResourceGroup(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "hosts.HostsClient", "ListByResourceGroup", result.HttpResponse, "Failure responding to request")
				return
			}
